...
export async function readJsonInput(options) {
    ...
    const sources = [options.data, options.file].filter((value) => value !== undefined);
    if (sources.length === 0) {
    ...
    try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        if (options.data !== undefined) {
            throw new Error(`Invalid ${label} JSON: ${message}. If using --data, make sure quotes inside strings are escaped (\\\") or removed.`);
        }
