...
export function resolveSkillSource(options: ResolveSkillSourceOptions): string {
  ...
  const walkUp = (startDir: string): string | null => {
    ...
    while (true) {
      if (existsSync(join(dir, marker))) return join(dir, "skills", skillName);
  ...
  throw new Error(
    `Could not locate skill source for '${skillName}' (expected ${marker}). Run from the repo checkout.`
  );
}

export async function installSkill(options: SkillInstallOptions): Promise<string> {
  const mode: SkillInstallMode = options.mode ?? "copy";
